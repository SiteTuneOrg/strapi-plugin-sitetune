...
describe("redirect-validation", () => {
  ...
  it("rejects a self-loop (from === to)", async () => {
    await expect(
      service([]).validateRedirectWrite({ from: "/same", to: "/same" })
    ).rejects.toThrow(/from.*to.*differ/i);
  });
  ...
  it("rejects a duplicate from value against another document", async () => {
    ...
    await expect(
      service(rows).validateRedirectWrite({ from: "/old", to: "/new" })
  ...
  it("rejects a direct 2-hop cycle (A->B, new B->A)", async () => {
    ...
    await expect(
      service(rows).validateRedirectWrite({ from: "/b", to: "/a" })
  ...
  it("rejects a longer cycle (A->B->C, new C->A)", async () => {
    ...
    await expect(
      service(rows).validateRedirectWrite({ from: "/c", to: "/a" })
    ).rejects.toThrow(/circular redirect/i);
  });
