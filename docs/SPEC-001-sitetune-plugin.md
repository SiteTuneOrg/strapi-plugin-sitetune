...
| OG/social | `spencercooley-strapi-plugin-og-pretty-link`, `littlebox-strapi-suite` | Neither generates images — `og-pretty-link` fetches OG data from *external* URLs, the opposite of our use case. |
...
- New collection type owned by the plugin, `plugin::sitetune.redirect`: `from` (string, required, unique), `to` (string, required), `statusCode` (enum `301`/`302`, default `301`), `enabled` (boolean, default `true`), `hitCount` (integer, optional, incremented if a frontend reports hits back).
- **Validation:** reject duplicate `from` values and circular redirects (`from === to`, or `to` pointing at another redirect's `from` in a way that loops) at the controller level before save.
- **Content-API endpoint:** `GET /sitetune/redirects` (public or token-gated, matching how the rest of this project's frontend already reads content) — returns the enabled redirect list for a frontend to enforce in its own middleware/edge function. CSV import via the admin UI for bulk migration scenarios (site relaunch, URL structure change).
- **Admin UI:** either a plain Content Manager collection-type view (simplest, free from Strapi's own CRUD UI, no custom admin code needed) or a dedicated plugin settings page if CSV import needs custom UI beyond what Content Manager's default import/export supports — decide during implementation, default to the plain Content Manager view unless CSV import specifically needs more.
- **Ships dark in v1, same as Pillar D (§3.5) and for the same reason (§1):** this plugin provides the data and the API; a specific site's frontend still has to fetch `/sitetune/redirects` and actually issue the 301/302 (e.g. Astro middleware, or a build step emitting a platform-specific redirects file). Not scheduled for any site here.
...
- **Component migration correctness (Pillar A)** — verify the bootstrap backfill is idempotent (running it twice doesn't duplicate/corrupt data) and doesn't clobber entries that already have `openGraph` data manually set.
...
**Pillar A — SEO + Social:**
- [ ] `shared.open-graph` component created; `openGraph` field added to `shared.seo`.
- [ ] i18n-ready field options set on both components.
- [ ] Idempotent migration backfills `openGraph` from existing `metaTitle`/`metaDescription`/`metaImage` for all existing `article`/`global` entries.
...
**Pillar C — Redirect Manager (ships dark in v1, see §3.4):**
- [ ] `plugin::sitetune.redirect` collection type with validation against duplicates and circular redirects.
- [ ] `GET /sitetune/redirects` content-API endpoint.
- [ ] CSV import working from the admin UI.
