...
14. **Code review of the Pillar C PR found four real gaps past what manual verification (item 13) happened to exercise — all fixed, none required a design change.** (a) The write-guard validated a trimmed `from`/`to` but forwarded the untrimmed value to the actual write, so a padded input like `" /old "` could validate clean and still persist with whitespace, undermining the very uniqueness/cycle guarantees just checked — fixed by writing the normalized value back before calling `next()`. (b) The guard only matched `create`/`update`; Content Manager's "Duplicate" button uses a third Document Service action, `clone` (verified against `@strapi/core`'s `services/document-service/repository.js`), which skipped validation entirely — `unique: true` incidentally caught the pure-duplicate case with a raw DB error, but an overridden `to` on a clone had no cycle protection at all. Fixed by guarding `clone` too, with the source document explicitly _not_ excluded from its own duplicate/cycle check (a clone is a new row, so matching its own source is a real conflict, not a self-edit). (c) A CSV row with an invalid `statusCode` (or an unrecognized `enabled` value) was silently coerced to a default instead of rejected — inconsistent with the same bad value getting a clean `400` on the Content Manager/content-API path; fixed to reject the row with a per-row error instead. (d) A leading UTF-8 byte-order-mark (Excel's "CSV UTF-8" export) broke header detection entirely, and header matching was case-sensitive — both fixed in the parser. A fifth, non-code finding: the uploaded file's temp copy was never deleted (koa-body's underlying `formidable` parser doesn't clean up after itself) — fixed with an explicit `unlink()` in a `finally` block. Two findings were investigated and found to be non-issues once checked against source rather than assumed: the admin fetch client's thrown `FetchError.message` already carries the API's real error message (verified in `@strapi/admin`'s `getFetchClient.js` — no fix needed in `RedirectImportForm.tsx`), and the cycle-check's `findMany()` calls aren't silently paginated (Strapi's REST default-limit-25 only applies to the REST controller's own `find` action, not a direct Document Service call — confirmed against `core-api/service/pagination.js`), so the O(n²) CSV-import cost documented as a known limitation is a round-trip-count concern, not a correctness one. See the README's Pillar C design notes for the full list with file references.
15. **A second code-review pass on the Pillar C PR, after end-to-end browser verification (item 13) already closed its own open item — found one real (if narrow) message-accuracy bug, confirmed two design choices as intentional rather than gaps.** `assertNoCycle`'s rejection message assumed the node that trips cycle detection is always the write's own `from` — true in the normal case (the very write closes the loop), but not if the walk instead runs into a pre-existing, unrelated cycle already sitting in the data (only reachable via the concurrent-write race documented in item 14, or an out-of-band edit) — the write is still correctly rejected either way (anyone following it would still loop forever), but the message would falsely claim `from` reappears when it never does. Fixed: the message now checks whether the offending node actually is `from` before wording it that way. The other two review points — `publicList` returning every enabled redirect unpaginated, and CSV upload size relying on Strapi's default body-parser limits rather than a route-specific cap — were confirmed as deliberate (the former; a paginated response would just push "fetch every page" onto every consumer of what's meant to be a one-shot lookup table) or accepted-but-unverified (the latter; no concrete size requirement exists yet to size a limit against) rather than fixed, and recorded as such in the README.
