...
- [ ] Second-boot idempotency **on a real host** — blocked by an unrelated local `ENOSPC` (inotify watcher limit) in this session's dev environment; covered by the equivalent unit test in the meantime. Re-verify against a real host when convenient.
