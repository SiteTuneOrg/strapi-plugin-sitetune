...
11. **Added `min`/`max` (1–5) to `testimonial.rating`.** Flagged in review: it's the only free-form numeric field across the four blocks — without bounds, `rating: 47` or `rating: -3` saves without error. `min`/`max` are the Strapi attribute keys for numeric fields (as opposed to `minLength`/`maxLength` for string/text, already used elsewhere in these schemas).
12. **Centralized the six-UID list into `constants.ts`'s `ALL_UIDS`, removing three separate hardcodings.** Before this, `isReady()` spelled out all six `strapi.components[UID] && ...` inline, and `schema-setup.test.ts` reconstructed the same list itself as a local `ALL_UIDS`, alongside `INDEPENDENT_COMPONENTS` in `schema-setup.ts` (which still needs its own `{ uid, schema }` pairing — not replaced by `ALL_UIDS`, since it needs the schema too). Flagged in review as a low-cost fix to avoid a fifth component someday updating `INDEPENDENT_COMPONENTS` but forgetting `isReady()` or the test's list. `isReady()` is now `ALL_UIDS.every((uid) => Boolean(strapi.components[uid]))`; the test imports the same `ALL_UIDS` instead of keeping its own copy.
13. **`team-member.email` switched from `"type": "string"` to Strapi's native `"type": "email"`** (confirmed valid — listed in both `@strapi/content-type-builder`'s and `@strapi/database`'s attribute-type constants) — free, well-supported format validation instead of none. **URL-shaped fields left as plain strings for now** (`testimonial.sourceUrl`, `cta.buttonUrl`, `team-member.linkedinUrl`/`.twitterUrl`/`.instagramUrl`), same convention as Pillar A's `canonicalURL`. Not fixed inline because it isn't a mechanical fix like `email`: `buttonUrl`/`canonicalURL` are deliberately allowed to hold relative paths (§2), so a shared "valid URL" regex would reject values these fields are meant to accept, and the social-link fields likely want a different (absolute-only) rule — real per-field design decisions, plus a question of whether to retrofit the already-shipped `canonicalURL`. Tracked in [SiteTuneOrg/strapi-plugin-sitetune#14](https://github.com/SiteTuneOrg/strapi-plugin-sitetune/issues/14) rather than decided ad hoc mid-review.
